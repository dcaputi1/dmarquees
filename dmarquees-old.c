...
int main(int argc, char **argv)
{
    ...
    {
        fprintf(stderr, "DRM master dropped — MAME can safely start.\n");
    }
    ...
    while (running)
    {
        ...
        char imgpath[512];
        snprintf(imgpath, sizeof(imgpath), "%s/%s.png", IMAGE_DIR, buf);
        struct stat st;
        if (stat(imgpath, &st) != 0)
        {
            fprintf(stderr, "image missing: %s\n", imgpath);
            continue;
        }
        if (game_has_multiple_screens(buf))
        {
            fprintf(stderr, "Skipping multi-screen game: %s\n", buf);
            continue;
        }
        int iw = 0, ih = 0;
        unsigned char *img = load_png_rgba(imgpath, &iw, &ih);
        if (!img)
        {
        ...
        if (fb_map)
        {
            ...
            // After writing to mmap'd framebuffer memory, the kernel should display it. If needed, call drmModeSetCrtc
            // again.
            if (drmModeSetCrtc(drm_fd, crtc_id, fb_id, 0, 0, &conn_id, 1, &chosen_mode) != 0)
            {
                if (errno == EPERM)
                    fprintf(stderr, "drmModeSetCrtc: Permission denied\n");
                else
    ...
    unlink(CMD_FIFO);
    fprintf(stderr, "dmarquees exiting\n");
    return 0;
}
